<template>
    <table id="dataTable" class="divide-y divide-gray-300">
        ...
        <tr>
            ...
            <th class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900" data-priority="3">Regions</th>
...
onMounted(() => {
    table = $('#dataTable').DataTable({
        ...
        ajax: async (data, callback) => {
            const response = await useCustomFetch('/admin/region', {
            ...
            {data: 'values', name: "Values"},
            {
                data: function (data) {
                    ...
                    if (actions.includes('edit')) {
                        buttons += '<button class="inline-flex items-center rounded-md border border-transparent bg-blue-600 px-3 py-2 text-sm font-medium leading-4 text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 region-edit-btn" data-id="' + data._id + '">Edit</button>'
                    }
                    buttons += '</div>'
                    return buttons;
    ...
    $(document).ready(() => {
        $(document).on('click', '.region-edit-btn', (e) => {
            e.preventDefault();
